...
class BaseLearner(nn.Module, metaclass=abc.ABCMeta):
    def __init__(self, model: nn.Module, args: argparse.Namespace):
        ...
        if not self.args.tune:
            self.ckpt_path = args.exp_path + '/{}_{}_sub_{}_seed_{}.pt'.format(args.encoder, args.data, args.cross_subject_id + 1, self.run_id)
            self.base_model_path = r'C:\Users\Lab2048\Desktop\SSVEP_Result\state_20\SSVEPFormer_benchmark_sub_{}.pt'.format(args.cross_subject_id + 1)
        else:
            # To avoid conflicts between multiple running trials
            self.ckpt_path = args.exp_path + '/{}_{}_sub_{}_seed_{}_{}.pt'.format(args.encoder, args.data, args.cross_subject_id, os.getpid(), self.run_id)
        ...
        # Initialize confusion matrix attributes (always, to avoid AttributeError in evaluate)
        self.y_pred_cf = []
        self.y_true_cf = []
    ...
    def plot_cf_matrix(self, path, classes, title=None):
        """绘制混淆矩阵
        ...
        Args:
            ...
            title: 图表标题（可选）
        """
        plot_confusion_matrix(self.y_true_cf, self.y_pred_cf, classes, path, title=title)

    def get_confusion_matrix(self, normalized=False):
        """返回混淆矩阵
        Args:
            normalized: 是否返回归一化的混淆矩阵
        Returns:
            cf_matrix: 混淆矩阵
        """
        cf_matrix = confusion_matrix(self.y_true_cf, self.y_pred_cf)
        if normalized:
