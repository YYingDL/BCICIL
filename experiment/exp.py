...
def offline_train_eval(task_stream, subjects, run, args):
    assert args.head != 'SplitCosineLinear'

    # Combine all the data in the task stream
    (x_train, y_train), (x_val, y_val), (x_test, y_test) = task_stream.setup_offline(load_subjects=subjects)
    ...
    early_stopping = EarlyStopping(path=ckpt_path, patience=args.patience, mode='max', verbose=False)
    ...
    for epoch in range(args.epochs):
        ...
        early_stopping(epoch_acc_val, model)
    ...
    if args.cf_matrix and args.tune is False:
        test_loss, test_acc, y_pred, y_true,cf_matrix = test_epoch_for_cf_matrix(model, test_loader, criterion, device=args.device)
        cf_matrix_path = args.exp_path + '/cf{}'.format(run)
        title = f'Run {run} - Subject {args.cross_subject_id} - {args.data}'
        plot_confusion_matrix(y_true, y_pred, path=cf_matrix_path, classes=np.arange(task_stream.n_classes), title=title)
    else:
    ...
    return val_acc, test_acc, cf_matrix
...
def experiment_multiple_runs(args):
    ...
    Acc_sub_run = []
    cf_matrix = None  # Initialize confusion matrix
    for run in range(args.runs):
        ...
        if args.agent == 'Offline':
            val_acc, test_acc, cf_matrix = offline_train_eval(task_stream,args.cross_subject_id, run, args)
            Acc_multiple_run_valid.append(val_acc)
            Acc_multiple_run_test.append(test_acc)
        else:
            task_stream.setup_subject(load_subjects=args.cross_subject_id)
            model = setup_model(args)
            ...
            for i in range(task_stream.n_tasks):
                ...
                test_acc = agent.evaluate(task_stream, path=tsne_path)  # TSNE path
                Acc_sub_run.append(test_acc)
                # Plot CF matrix after finishing the final task.
                if i + 1 == task_stream.n_tasks and args.cf_matrix:
                    cf_matrix_path = args.exp_path + '/cf{}'.format(run)
                    title = f'Run {run} - Subject {args.cross_subject_id} - {args.data} ({args.agent})'
                    agent.plot_cf_matrix(path=cf_matrix_path, classes=np.arange(task_stream.n_classes), title=title)
            Acc_multiple_run_valid.append(agent.Acc_tasks['valid'])
            Acc_multiple_run_test.append(agent.Acc_tasks['test'])

            # Get confusion matrix from the last run for averaging
            if run == args.runs - 1 and args.cf_matrix:
                cf_matrix = agent.get_confusion_matrix(normalized=False)  # 返回原始计数
            else:
                cf_matrix = None
    ...
    return Acc_multiple_run_test, cf_matrix
