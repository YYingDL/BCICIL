...
import os
from agents.utils.name_match import agents_replay, agents_analytic
...
config_cl = {'LwF': {'lambda_kd_lwf': tune.grid_search([1, 1e1, 1e2, 1e3, 1e4]),
             ...
             'DERS': {'der_plus': False, 'der_dynamic': True, 'der_aux': True},
             ...
             'ADR': {'adr_buffer_size': tune.grid_search([256, 512, 1024]),
                     'adr_gamma': tune.grid_search([1e-4, 1e-3, 1e-2]),
                      'base_epochs': 50  # Base training epochs for first task
                      }
             }
...
def modify_config_accordingly(args, config_generic, config_model, config_cl):
    ...
    # ADR: Analytic learning methods use different settings
    if args.agent in agents_analytic:
        # ADR doesn't use replay buffer in the traditional sense
        # Uses random projection buffer instead
        args.patience = 10  # Moderate patience for base training
        # ADR uses torch.double for numerical stability
        # Note: This may need custom handling in the training loop
