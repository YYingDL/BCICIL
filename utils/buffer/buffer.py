...
from utils.setup_elements import get_buffer_size, n_classes_init_task, get_num_classes,n_tasks
...
class Buffer(torch.nn.Module):
    def __init__(self, model, args, mem_size=None):
        ...
        self.params.num_tasks = n_tasks[args.data]
        ...
        buffer_logits = torch.FloatTensor(self.mem_size, n_classes[args.data]).fill_(0).to(self.device) if args.agent == 'DER' else None
